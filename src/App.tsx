...
function AppearanceEditor({ view, store, onDone }: any) {
  ...
  return <div className="appearance-stack dialog-stack"><p className="section-kicker">Estilo del diagrama</p><div className="theme-grid">{([['academic', 'Académico', 'paper'], ['warm', 'Cálido', 'warm'], ['modern', 'Moderno', 'modern']] as const).map(([id, label, klass]) => <button type="button" className={`theme-card ${view.theme === id ? 'selected' : ''}`} key={id} onClick={() => store.setTheme(id)}><span className={`theme-preview ${klass}`}><b /><i /></span><span>{label}</span></button>)}</div><div className="layout-mode-section"><p className="section-kicker">Distribución</p><div className="segmented layout-mode-toggle" role="group" aria-label="Modo de distribución del diagrama"><button type="button" className={layoutMode === 'structured' ? 'active' : ''} aria-pressed={layoutMode === 'structured'} onClick={() => store.setLayoutMode('structured')}>Estructurado</button><button type="button" className={layoutMode === 'freeform' ? 'active' : ''} aria-pressed={layoutMode === 'freeform'} onClick={() => store.setLayoutMode('freeform')}>Libre</button></div><p className="layout-mode-help">Estructurado ajusta entidades y relaciones a una cuadrícula de 24 px.</p></div><div className="cardinality-placement-section"><p className="section-kicker">Ubicación de cardinalidad</p><div className="segmented cardinality-placement-toggle" role="group" aria-label="Ubicación de las cardinalidades"><button type="button" className={cardinalityPlacement === 'near-entity' ? 'active' : ''} aria-pressed={cardinalityPlacement === 'near-entity'} onClick={() => store.setCardinalityPlacement('near-entity')}>Junto a su entidad</button><button type="button" className={cardinalityPlacement === 'opposite-entity' ? 'active' : ''} aria-pressed={cardinalityPlacement === 'opposite-entity'} onClick={() => store.setCardinalityPlacement('opposite-entity')}>En el extremo opuesto</button></div><p className="layout-mode-help">Cambia qué cardinalidad aparece en cada extremo.</p></div><div className="custom-toggle"><span>Colores personalizados</span><small>Opcional</small></div><div className="color-fields">{([['background', 'Fondo'], ['entity', 'Entidad'], ['relationship', 'Relación'], ['ink', 'Tinta']] as const).map(([key, label]) => <label key={key}>{label}<input type="color" value={custom[key]} onChange={e => { const next = { ...custom, [key]: e.target.value }; setCustom(next); store.setTheme('custom'); store.updateCustomTheme(next) }} /></label>)}</div><label className="font-select">Tipografía<select value={custom.font} onChange={e => { const font = e.target.value as 'serif' | 'sans'; const next = { ...custom, font }; setCustom(next); store.setTheme('custom'); store.updateCustomTheme(next) }}><option value="serif">Serif académica</option><option value="sans">Sans moderna</option></select></label><button type="button" className="secondary-button full-button dialog-submit" onClick={onDone}>Listo</button></div>
}
